...
class ActorNetwork(nn.Module):
    def __init__(self, n_actions, input_dims, alpha, std, fc1_dims=256,
                fc2_dims=256, chkpt_dir='PPO_chkpts'):
    ...
    def forward(self, state):
      mu= self.actor(state)
      dist = T.distributions.Normal(mu, self.std)

      return dist

    def save_checkpoint(self):
      is_exist= os.path.exists(self.chkpt_dir)
...
class Agent:
    def __init__(self, n_actions, input_dims, gamma=0.99, alpha=0.0003, 
                gae_lambda=0.95, std=0.4, policy_clip=0.2, batch_size=64,
                N=2048, n_epochs=10):
      self.gamma = gamma
      ...
      self.actor = ActorNetwork(n_actions, input_dims, alpha, self.std)
...
plt_file_name="PPO_progress_plot" + str(i+1) + ".png"
fig1.savefig(plt_file_name)



# # gen video
print("Testing and video generation")
env= gym.make('InvertedPendulum-v2')
...
result = cv2.VideoWriter('invertedPendulum_PPO.avi',cv2.VideoWriter_fourcc(*'XVID'),fps,size)
